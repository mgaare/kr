(ns com.michaelgaare.kr.sesame.rdf
  ...
  ([kb s type-or-lang]
     ...
     (.createLiteral (:value-factory kb)
                     ...
                     (if (string? type-or-lang)
                       type-or-lang
                       (com.michaelgaare.kr.rdf/resource kb type-or-lang)))))
...
(defn sesame-context-array
  ...
  ([kb c] (if c
            (let [a (make-array Resource 1)]
              (aset a 0 ^Resource (com.michaelgaare.kr.rdf/resource kb c))
              ;;(sesame-uri (resource-ify c)))
              a)
            (sesame-context-array)))
  ([kb c & rest] (let [a (make-array Resource (+ 1 (count rest)))]
                   (map (fn [v i]
                          (aset a i (com.michaelgaare.kr.rdf/resource kb v)))
                     (cons c rest)
                     (range))
                a)))
...
(defn sesame-ask-statement
  ([kb s p o context]
     (.hasStatement ^RepositoryConnection (connection! kb)
                    ^Resource (and s (com.michaelgaare.kr.rdf/resource kb s))
